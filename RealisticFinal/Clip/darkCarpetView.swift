...
class AppStateDC: ObservableObject {
    @Published var simpleFlag = false
}
 
struct  darkCarpetView: UIViewControllerRepresentable {
//    @ObservedObject var fsettings : FloorSettings
    typealias UIViewControllerType = darkCarpetController
    
    @EnvironmentObject var settings: AppStateDC
    
    func makeUIViewController(context: Context) -> darkCarpetController {
        let  darkCarpetViewController = UIViewControllerType()
//         darkCarpetViewController.fsettings = fsettings
//        let  darkCarpetViewController = UIStoryboard(name: "Test", bundle: nil).instantiateViewController(identifier: String(describing: darkCarpetController.self)) as! darkCarpetController
//         darkCarpetViewController.settings = fsettings
//          darkCarpetView.delegate = context.coordinator
        return  darkCarpetViewController
    }
    ...
    func makeCoordinator() ->  darkCarpetView.Coordinator {
        return Coordinator(self)
    }
}

extension  darkCarpetView {
    class Coordinator /*: SomeUIKitViewDelegate */ {
        var parent:  darkCarpetView
        
        init(_ parent:  darkCarpetView) {
            self.parent = parent
        }
...
struct  darkCarpetView_Previews: PreviewProvider {
    static var previews: some View {
         darkCarpetView()
    }
}

class darkCarpetController: UIViewController, UICollisionBehaviorDelegate {
    
    var fullFloorImg: String = "easyfit-full"
...
//        floorTexture = settings.ahapName
    ...
    private func initializeBackgroundImage() {
        let backgroundImagePath = Bundle.main.path(forResource: "easyfit-full",
                                                   ofType: "jpeg")!
...
extension darkCarpetController {
