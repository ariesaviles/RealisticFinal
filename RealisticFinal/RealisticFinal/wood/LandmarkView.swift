...
class AppStateWLW: ObservableObject {
    @Published var simpleFlag = false
}

struct  LandmarkWView: UIViewControllerRepresentable {
//    @ObservedObject var fsettings : FloorSettings
    typealias UIViewControllerType = LandmarkWController
    
    @EnvironmentObject var settings: AppStateWLW
    
    func makeUIViewController(context: Context) -> LandmarkWController {
        let  LandmarkWViewController = UIViewControllerType()
//         LandmarkWViewController.fsettings = fsettings
//        let  LandmarkWViewController = UIStoryboard(name: "Test", bundle: nil).instantiateViewController(identifier: String(describing: LandmarkWController.self)) as! LandmarkWController
//         LandmarkWViewController.settings = fsettings
//          LandmarkWView.delegate = context.coordinator
        return  LandmarkWViewController
    }
    ...
    func makeCoordinator() ->  LandmarkWView.Coordinator {
        return Coordinator(self)
    }
}

extension  LandmarkWView {
    class Coordinator /*: SomeUIKitViewDelegate */ {
        var parent:  LandmarkWView
        
        init(_ parent:  LandmarkWView) {
            self.parent = parent
        }
...
struct  LandmarkWView_Previews: PreviewProvider {
    static var previews: some View {
         LandmarkWView()
    }
}

class LandmarkWController: UIViewController, UICollisionBehaviorDelegate {
    
    var fullFloorImg: String = "landmarkwalnut-full"
