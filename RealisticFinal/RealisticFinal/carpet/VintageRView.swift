...
class AppStateCVR: ObservableObject {
    @Published var simpleFlag = false
}

struct  VintageRView: UIViewControllerRepresentable {
//    @ObservedObject var fsettings : FloorSettings
    typealias UIViewControllerType = VintageRController
    
    @EnvironmentObject var settings: AppStateCVR
    
    func makeUIViewController(context: Context) -> VintageRController {
        let  VintageRViewController = UIViewControllerType()
//         VintageRViewController.fsettings = fsettings
//        let  VintageRViewController = UIStoryboard(name: "Test", bundle: nil).instantiateViewController(identifier: String(describing: VintageRController.self)) as! VintageRController
//         VintageRViewController.settings = fsettings
//          VintageRView.delegate = context.coordinator
        return  VintageRViewController
    }
    ...
    func makeCoordinator() ->  VintageRView.Coordinator {
        return Coordinator(self)
    }
}

extension  VintageRView {
    class Coordinator /*: SomeUIKitViewDelegate */ {
        var parent:  VintageRView
        
        init(_ parent:  VintageRView) {
            self.parent = parent
        }
...
struct  VintageRView_Previews: PreviewProvider {
    static var previews: some View {
         VintageRView()
    }
}

class VintageRController: UIViewController, UICollisionBehaviorDelegate {
    
    var fullFloorImg: String = "vintagerevival-full"
