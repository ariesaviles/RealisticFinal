...
class AppStateCCS: ObservableObject {
    @Published var simpleFlag = false
}

struct  CalmSView: UIViewControllerRepresentable {
//    @ObservedObject var fsettings : FloorSettings
    typealias UIViewControllerType = CalmSController
    
    @EnvironmentObject var settings: AppStateCCS
    
    func makeUIViewController(context: Context) -> CalmSController {
        let  CalmSViewController = UIViewControllerType()
//         CalmSViewController.fsettings = fsettings
//        let  CalmSViewController = UIStoryboard(name: "Test", bundle: nil).instantiateViewController(identifier: String(describing: CalmSController.self)) as! CalmSController
//         CalmSViewController.settings = fsettings
//          CalmSView.delegate = context.coordinator
        return  CalmSViewController
    }
    ...
    func makeCoordinator() ->  CalmSView.Coordinator {
...
extension  CalmSView {
    class Coordinator /*: SomeUIKitViewDelegate */ {
        var parent:  CalmSView
        
        init(_ parent:  CalmSView) {
            self.parent = parent
        }
...
struct  CalmSView_Previews: PreviewProvider {
    static var previews: some View {
         CalmSView()
    }
}

class CalmSController: UIViewController, UICollisionBehaviorDelegate {
    
    var fullFloorImg: String = "calmserenity-full"
