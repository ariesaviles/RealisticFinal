...
class AppStateTI: ObservableObject {
    @Published var simpleFlag = false
}

struct  IslanderView: UIViewControllerRepresentable {
//    @ObservedObject var fsettings : FloorSettings
    typealias UIViewControllerType = IslanderController
    
    @EnvironmentObject var settings: AppStateTI
    
    func makeUIViewController(context: Context) -> IslanderController {
        let  IslanderViewController = UIViewControllerType()
//         IslanderViewController.fsettings = fsettings
//        let  IslanderViewController = UIStoryboard(name: "Test", bundle: nil).instantiateViewController(identifier: String(describing: IslanderController.self)) as! IslanderController
//         IslanderViewController.settings = fsettings
//          IslanderView.delegate = context.coordinator
        return  IslanderViewController
    }
    ...
    func makeCoordinator() ->  IslanderView.Coordinator {
        return Coordinator(self)
    }
}

extension  IslanderView {
    class Coordinator /*: SomeUIKitViewDelegate */ {
        var parent:  IslanderView
        
        init(_ parent:  IslanderView) {
            self.parent = parent
        }
...
struct  IslanderView_Previews: PreviewProvider {
    static var previews: some View {
         IslanderView()
    }
}

class IslanderController: UIViewController, UICollisionBehaviorDelegate {
    
    var fullFloorImg: String = "islander-full"
