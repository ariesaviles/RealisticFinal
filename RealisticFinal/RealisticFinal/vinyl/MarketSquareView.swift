...
class AppStateMS: ObservableObject {
    @Published var simpleFlag = false
}

struct  MarketSquareView: UIViewControllerRepresentable {
//    @ObservedObject var fsettings : FloorSettings
    typealias UIViewControllerType = MarketSquareController
    
    @EnvironmentObject var settings: AppStateMS
    
    func makeUIViewController(context: Context) -> MarketSquareController {
        let  MarketSquareViewController = UIViewControllerType()
//         MarketSquareViewController.fsettings = fsettings
//        let  MarketSquareViewController = UIStoryboard(name: "Test", bundle: nil).instantiateViewController(identifier: String(describing: MarketSquareController.self)) as! MarketSquareController
//         MarketSquareViewController.settings = fsettings
//          MarketSquareView.delegate = context.coordinator
        return  MarketSquareViewController
    }
    ...
    func makeCoordinator() ->  MarketSquareView.Coordinator {
        return Coordinator(self)
    }
}

extension  MarketSquareView {
    class Coordinator /*: SomeUIKitViewDelegate */ {
        var parent:  MarketSquareView
        
        init(_ parent:  MarketSquareView) {
            self.parent = parent
        }
...
struct  MarketSquareView_Previews: PreviewProvider {
    static var previews: some View {
         MarketSquareView()
    }
}

class MarketSquareController: UIViewController, UICollisionBehaviorDelegate {
    
    var fullFloorImg: String = "marketsquare-full"
    var wallTexture: String = "CollisionSmall"
    var floorTexture: String = "Texture-solid"
...
extension MarketSquareController {
