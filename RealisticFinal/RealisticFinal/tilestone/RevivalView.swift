...
class AppStateRD: ObservableObject {
    @Published var simpleFlag = false
}

struct  RevivalView: UIViewControllerRepresentable {
//    @ObservedObject var fsettings : FloorSettings
    typealias UIViewControllerType = RevivalController
    
    @EnvironmentObject var settings: AppStateRD
    
    func makeUIViewController(context: Context) -> RevivalController {
        let  RevivalViewController = UIViewControllerType()
//         RevivalViewController.fsettings = fsettings
//        let  RevivalViewController = UIStoryboard(name: "Test", bundle: nil).instantiateViewController(identifier: String(describing: RevivalController.self)) as! RevivalController
//         RevivalViewController.settings = fsettings
//          RevivalView.delegate = context.coordinator
        return  RevivalViewController
    }
    ...
    func makeCoordinator() ->  RevivalView.Coordinator {
        return Coordinator(self)
    }
}

extension  RevivalView {
    class Coordinator /*: SomeUIKitViewDelegate */ {
        var parent:  RevivalView
        
        init(_ parent:  RevivalView) {
            self.parent = parent
        }
...
struct  RevivalView_Previews: PreviewProvider {
    static var previews: some View {
         RevivalView()
    }
}

class RevivalController: UIViewController, UICollisionBehaviorDelegate {
    
    var fullFloorImg: String = "revivalmaria-full"
