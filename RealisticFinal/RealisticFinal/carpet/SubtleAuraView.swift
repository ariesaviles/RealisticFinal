...
class AppStateSA: ObservableObject {
    @Published var simpleFlag = false
}

struct SubtleAView: UIViewControllerRepresentable {
//    @ObservedObject var fsettings : FloorSettings
    typealias UIViewControllerType = SubtleAController
    
    @EnvironmentObject var settings: AppStateSA
    
    func makeUIViewController(context: Context) -> SubtleAController {
        let SubtleAViewController = UIViewControllerType()
//        SubtleAViewController.fsettings = fsettings
//        let SubtleAViewController = UIStoryboard(name: "Test", bundle: nil).instantiateViewController(identifier: String(describing: SubtleAController.self)) as! SubtleAController
//        SubtleAViewController.settings = fsettings
//         SubtleAView.delegate = context.coordinator
        return SubtleAViewController
    }
    ...
    func makeCoordinator() -> SubtleAView.Coordinator {
        return Coordinator(self)
    }
}

extension SubtleAView {
    class Coordinator /*: SomeUIKitViewDelegate */ {
        var parent: SubtleAView
        
        init(_ parent: SubtleAView) {
            self.parent = parent
        }
...
struct SubtleAView_Previews: PreviewProvider {
    static var previews: some View {
        SubtleAView()
    }
}

class SubtleAController: UIViewController, UICollisionBehaviorDelegate {
    
    var fullFloorImg: String = "subtleaura-full"
