...
class AppStateVP: ObservableObject {
    @Published var simpleFlag = false
}

struct  ParagonView: UIViewControllerRepresentable {
//    @ObservedObject var fsettings : FloorSettings
    typealias UIViewControllerType = ParagonController
    
    @EnvironmentObject var settings: AppStateVP
    
    func makeUIViewController(context: Context) -> ParagonController {
        let  ParagonViewController = UIViewControllerType()
//         ParagonViewController.fsettings = fsettings
//        let  ParagonViewController = UIStoryboard(name: "Test", bundle: nil).instantiateViewController(identifier: String(describing: ParagonController.self)) as! ParagonController
//         ParagonViewController.settings = fsettings
//          ParagonView.delegate = context.coordinator
        return  ParagonViewController
    }
    ...
    func makeCoordinator() ->  ParagonView.Coordinator {
        return Coordinator(self)
    }
}

extension  ParagonView {
    class Coordinator /*: SomeUIKitViewDelegate */ {
        var parent:  ParagonView
        
        init(_ parent:  ParagonView) {
            self.parent = parent
        }
...
struct  ParagonView_Previews: PreviewProvider {
    static var previews: some View {
         ParagonView()
    }
}

class ParagonController: UIViewController, UICollisionBehaviorDelegate {
    
    var fullFloorImg: String = "paragonmix-full"
