...
class AppStateWEO: ObservableObject {
    @Published var simpleFlag = false
}

struct  EmpireOakView: UIViewControllerRepresentable {
//    @ObservedObject var fsettings : FloorSettings
    typealias UIViewControllerType = EmpireOakController
    
    @EnvironmentObject var settings: AppStateWEO
    
    func makeUIViewController(context: Context) -> EmpireOakController {
        let  EmpireOakViewController = UIViewControllerType()
//         EmpireOakViewController.fsettings = fsettings
//        let  EmpireOakViewController = UIStoryboard(name: "Test", bundle: nil).instantiateViewController(identifier: String(describing: EmpireOakController.self)) as! EmpireOakController
//         EmpireOakViewController.settings = fsettings
//          EmpireOakView.delegate = context.coordinator
        return  EmpireOakViewController
    }
    ...
    func makeCoordinator() ->  EmpireOakView.Coordinator {
        return Coordinator(self)
    }
}

extension  EmpireOakView {
    class Coordinator /*: SomeUIKitViewDelegate */ {
        var parent:  EmpireOakView
        
        init(_ parent:  EmpireOakView) {
            self.parent = parent
        }
...
struct  EmpireOakView_Previews: PreviewProvider {
    static var previews: some View {
         EmpireOakView()
    }
}

class EmpireOakController: UIViewController, UICollisionBehaviorDelegate {
    
    var fullFloorImg: String = "empireoak-full"
